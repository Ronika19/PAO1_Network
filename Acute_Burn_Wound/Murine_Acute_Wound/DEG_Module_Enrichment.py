...
class DEG_Enrichment:
	def DataPreprocess(self, outfile):
		...
		infile_list = ['Murine_Dataset_UpRegulatedDEGs.txt', 'Murine_Dataset_DownRegulatedDEGs.txt']
		...
		fl_list = ['Pathogenesis_Vs_BiologicalReplicate.txt', 'PAO1_Upregulated_DEGs.txt', 'PAO1_Downregulated_DEGs.txt', 'PAO1_Cluster_DEG_BiologicalReplicates.txt']
		Cluster = Data_Processer.DEG_Cluster(fl_list[0], fl_list[1], fl_list[2], fl_list[3])
	
		mod_clustlist = ['Module_Assignment_PAO1_WGCNA.tsv', 'PAO1_Modules_Clusters.txt', 'WGCNA_PAO1_Modules_Clusters.txt']
		mod_cluster = Data_Processer.Modules_Cluster(mod_clustlist[0], mod_clustlist[1], mod_clustlist[2])

		clus_mod_list = ['PAO1_Cluster_DEG_BiologicalReplicates.txt', 'Module_Assignment_PAO1_WGCNA.tsv', 'PAO1_DEG_Modules.txt', 'PAO1_DEG_Modules.txt', 'PAO1_DEG_Genes_Modules.txt']
	...
	def Enrichment(self, infile1, infile2, infile3, infile4, outfile):
		self.DataPreprocess('Pathogenesis_Vs_BiologicalReplicate.txt')
